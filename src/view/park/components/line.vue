...
const chartOptions = ref({
    ...
    tooltip: {
        ...
        pointFormatter: function () {
            ...
            return `<b>${key}</b>&nbsp&nbsp&nbsp&nbsp${this.y}${unit} &nbsp&nbsp&nbsp ${status} `;
        }
    },
    credits: {
        enabled: false
...
const updateChartOptions = (dataList, categories, isExcellent) => {
    ...
    //更新series数据
    chartOptions.value.series = Object.keys(dataList).slice(0, 4).map(key => {
...
const updateYAxis = (globalMin, globalMax) => {
    ...
    const adjustedMin = globalMin < 0 ? Math.floor(globalMin / 1.2) : 0;
    const range = adjustedMax - adjustedMin;
    const tickInterval = Math.ceil(range / 3);

    // chartOptions.value.yAxis.min = adjustedMin < 0 ? adjustedMin : 0;  // 如果最小值不为负则设置为0
    // chartOptions.value.yAxis.max = adjustedMax;  // 设置最大值
    // chartOptions.value.yAxis.tickInterval = tickInterval;  // 设置刻度间隔
